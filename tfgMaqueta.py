...
#establecer comunicación ADS
#ads_net_id = '192.168.254.1.1.1'   #COMPROBAR
ads_net_id ='192.168.0.121.1.1'
...
def funcionMano(resultados):
    ...
    if len(resultados[0].boxes.xywh) != 0:
        for box in resultados[0].boxes.xywh:
            x_central = box[0].item()           # calculo la x_central (es lo más fácil) y comparo con el centro de la imagen para saber si es izq o der
            if x_central < 320:
                Der = True
            elif x_central > 320:
                Izq = True
...
print('Conetando con TwinCAT...')
plc.open()
write_dict = {'GVL.bHeartBeat':0}
plc.write_list_by_name(write_dict)

print('Estado de la conexión:', plc.is_open)
print('Estado del programa:', plc.read_state())  # si devuelve (5,0) está ok
# 5 = RUN mode
# 0 = no error

var_list = ['GVL.gIzq', 'GVL.gDer', 'GVL.alpha','GVL.centr','GVL.Av','GVL.Frena','GVL.borrado',
            'GVL.bFinPrograma','GVL.IHM_StopOp','GVL.bHeartBeat']
plc.read_list_by_name(var_list)
print("Empezando videocaptura")

# empezamos con la videocaptura
while True:
    game_over=plc.read_by_name(data_name="GVL.B_GAME_OVER",plc_datatype=pyads.PLCTYPE_BOOL)

    #print(game_over)

    write_dict = {'GVL.bFinPrograma':False,'GVL.bHeartBeat':1}
    plc.write_list_by_name(write_dict)
    ...
    if not ret or not plc.is_open:
        if not ret:
            print("La captura está fallando")
        if not plc.is_open:
            print("Error de comunicación, reiniciar configuración twincat")
        write_dict = {'GVL.gIzq':False, 'GVL.gDer':False, 'GVL.alpha':0.0,'GVL.centr':False,
                    'GVL.Av':False,'GVL.Frena':False,'GVL.borrado':False,'GVL.bFinPrograma':True,'GVL.IHM_StopOp':False,'GVL.bHeartBeat':0}
        plc.write_list_by_name(write_dict)
        break
    # Leemos resultados y los vamos guardando
    resultados = model.predict(frame, imgsz=640,conf=0.5)  # anotamos resultados con la confidencia adecuada para la detección
    ...
    if 2 not in detectedCLS: #reinicio volante
        ...
        prim = 0
        write_dict = {'GVL.gIzq': False, 'GVL.gDer': False, 'GVL.alpha': 0.0, 'GVL.centr': False,
                      'GVL.Av': False, 'GVL.Frena': False, 'GVL.borrado': False,
                      'GVL.bFinPrograma': False,'GVL.IHM_StopOp':False,'GVL.bHeartBeat':1}
        plc.write_list_by_name(write_dict)
            ...
            if results[0].boxes.id is not None and results[0].masks is not None:
                ...
                for mask, track_id in zip(masks, track_ids):
                    ...
                    if len(mask) > 0:
                        ...
                        angulo=math.degrees(alpha)
                        print("angulo: ",angulo)
                        write_dict = {'GVL.gIzq': gIzq, 'GVL.gDer': gDer, 'GVL.alpha': angulo, 'GVL.centr': centrado,
                                      'GVL.Av': Avance, 'GVL.Frena': Frena, 'GVL.borrado': False,
                                      'GVL.bFinPrograma': False,'GVL.IHM_StopOp':False,'GVL.bHeartBeat':1}
                        plc.write_list_by_name(write_dict)
...
#################      MANOS   #########################
            ...
            if (izquierda or derecha) and not (izquierda and derecha):
                ...
                if cStop==10:
                    cStop=0
                    bStop=True
                    write_dict = {'GVL.gIzq': False, 'GVL.gDer': False, 'GVL.alpha': angulo, 'GVL.centr': False,
                                  'GVL.Av': False, 'GVL.Frena': False, 'GVL.borrado': False,
                                  'GVL.bFinPrograma': False, 'GVL.IHM_StopOp': True,'GVL.bHeartBeat':1}
                    plc.write_list_by_name(write_dict)
            ...
            if izquierda and derecha:
                ...
                bBorrado=True
                write_dict = {'GVL.gIzq': False, 'GVL.gDer': False, 'GVL.alpha': angulo, 'GVL.centr': False,
                              'GVL.Av': False, 'GVL.Frena': False, 'GVL.borrado': True,
                              'GVL.bFinPrograma': False,'GVL.IHM_StopOp':False,'GVL.bHeartBeat':1}
                plc.write_list_by_name(write_dict)
...
#################      VOLANTE    #########################
            ...
            if prim == 0:
                ...
                if marca1 and marca2:
                    ...
                    print("angulo: ", angulo)
                    write_dict = {'GVL.gIzq': gIzq, 'GVL.gDer': gDer, 'GVL.alpha': angulo, 'GVL.centr': centrado,
                                  'GVL.Av': Avance, 'GVL.Frena': Frena, 'GVL.borrado': False, 'GVL.bFinPrograma': False
                                  ,'GVL.IHM_StopOp':False,'GVL.bHeartBeat':1}
                    plc.write_list_by_name(write_dict)
                marca1 = False
                marca2 = False
    ...
    for Ctxt in range(len(consignas_text)):
        cv2.putText(imag, consignas_text[Ctxt], (50, (1+Ctxt)*50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
    ...
    if cv2.waitKey(1) == 27:
        write_dict = {'GVL.gIzq': False, 'GVL.gDer': False, 'GVL.alpha': 0.0, 'GVL.centr': False,
                      'GVL.Av': False, 'GVL.Frena': False, 'GVL.borrado': False, 'GVL.bFinPrograma': True
            , 'GVL.IHM_StopOp': False, 'GVL.bHeartBeat': 0}
        plc.write_list_by_name(write_dict)
        break
...
write_dict = {'GVL.gIzq': False, 'GVL.gDer': False, 'GVL.alpha': 0.0, 'GVL.centr': False,
                      ...
                      ,'GVL.IHM_StopOp':False,'GVL.bHeartBeat':0}
plc.write_list_by_name(write_dict)
# cierro la conexión con Twincat
print('Cerrando conexión con Twincat...')
plc.close()
print('Estado de la conexión :', plc.is_open)